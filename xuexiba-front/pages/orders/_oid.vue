    ...
    <form name="flowForm" id="flowForm" method="post" action="">
      ...
      <div class="Finish">
        <div class="fr" id="AgreeDiv">
          <label for="Agree"
            ><p class="on">
              <input type="checkbox" checked="checked"  @change="disablechange"/>我已阅读并同意<a
                href="javascript:"
                target="_blank"
                >《学习吧购买协议》</a
              >
        ...
        <button :disabled="dis" target="_blank" type="button" id="submitPay" @click="toPay()">
          去支付
        </button>
        <div class="clear"></div>
      </div>
    </form>
    <dev ref="alipayWap" v-html="alipay"  target="_blank"> </dev>
  </div>
...
export default {
  ...
  // },
  created(){
    this.idss=this.$route.params.oid
      this.orderInfo()
  },
  methods: {  
      
    orderInfo(){
      ordersApi.getOrdersInfo(this.idss).then(response=>{
    ...
    toPay() {
      ...
      ordersApi.payOrder(this.order.orderNo).then(response=>{
        this.alipay=response.data.data.result
       this.$nextTick(()=>{
